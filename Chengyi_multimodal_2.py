...
from tensorflow.python.keras.initializers import GlorotUniform, GlorotNormal, constant, RandomNormal
...
stat=sio.loadmat('stat45.mat')  #includes patient ID, classification
img=sio.loadmat('img45.mat')    #image MRI/PET
numex = pd.read_excel('ADNIMERGE2.xlsx',index_col = 0,header=0)
...
statarr = np.zeros((len(stat), 3))    #diagnose from stats45
numarr = np.zeros((len(stat),numsize))
statarr2 =[] #the last diagnose
randStat = [] #random
count = 0
...
conv=(Dense(1024, activation = 'relu',  kernel_initializer = GlorotUniform(),
